...
s = sqrt(s)
// For x = 190 cm/s
y1 = 1.79 - ( (t_thr)*s* sqrt(1 + 1.0/10 + (air_vel - sum(x)/10.0)^2/(s1) ) )
y2 = 1.79 + ( (t_thr)*s* sqrt(1 + 1.0/10 + (air_vel - sum(x)/10.0)^2/(s1) ) )

// Result
printf ( "95%% confidence interval(in mm(square)/s): ( %.2f , %.2f )",y1,y2)
